...
class MainWindow(QMainWindow):
    ...
    def _init_ui(self):
        ...
        # 可视化分析图（支持交互式主体框选）
        vis_card = CardWidget("可视化分析（可在图上拖拽框选主体区域，滚轮缩放）")
        self.vis_scroll = QScrollArea()
        self.vis_scroll.setWidgetResizable(False)
        self.vis_scroll.setAlignment(Qt.AlignCenter)
        self.vis_scroll.setMinimumHeight(400)
        self.vis_scroll.setStyleSheet(f"QScrollArea {{ background-color: {Colors.SIDEBAR_BG}; border: 1px solid {Colors.BORDER_LIGHT}; border-radius: 8px; }}")
        self.overlay_label = SelectableImageLabel()
        self.overlay_label.setAlignment(Qt.AlignCenter)
        self.overlay_label.setStyleSheet(f"background-color: {Colors.SIDEBAR_BG}; color: {Colors.TEXT_LIGHT}; font-size: 14px;")
        self.overlay_label.setText("分析后将在此显示构图叠加图\n可在图上拖拽框选主体区域进行聚焦分析")
        self.overlay_label.roi_selected.connect(self._on_roi_selected)
        self.vis_scroll.setWidget(self.overlay_label)
        vis_card.add_widget(self.vis_scroll)
