...
class SelectableImageLabel(QLabel):
    ...
    def _update_display(self):
        ...
        base_scaled = source.scaled(self.size(), Qt.KeepAspectRatio, Qt.SmoothTransformation)
        zw = int(base_scaled.width() * self._zoom)
        zh = int(base_scaled.height() * self._zoom)
        scaled = self._pixmap_orig.scaled(zw, zh, Qt.KeepAspectRatio, Qt.SmoothTransformation)
        # 计算图片在label中的位置（居中+平移偏移）
        ...
        if self._roi_rect is not None:
            ...
            # 清除ROI区域的遮罩
            local_roi = QRect(
                self._roi_rect.x() - self._img_rect.x(),
                self._roi_rect.y() - self._img_rect.y(),
                self._roi_rect.width(),
                self._roi_rect.height()
            )
    ...
    def wheelEvent(self, event):
        ...
        self._pan_offset = QPoint(
            int(self._pan_offset.x() * ratio),
            int(self._pan_offset.y() * ratio)
        )
        self._update_display()
        event.accept()
